...
jwin = Counter(v['winner'] for v in judge_clean.values())
...
# H5: LLM judge — dist score >= raw score (paired t-test)
if len(jrs) >= 5:
    ...
    results['H5'] = dict(stat=t5,p=p5,d=d5,ci=ci5,n=len(jrs),
        name="LLM judge: dist score >= raw score",
        dim="QUALITY",method="Paired t-test (judge=llama-3.3-70b-versatile, one-tailed)",
        sig=p5<ALPHA)
else:
    results['H5'] = dict(stat=None,p=None,d=None,ci=None,n=len(jrs),
        name="LLM judge: dist score >= raw score",
        dim="QUALITY",method="Paired t-test — PENDING (run score_and_judge_v3.py)",
        sig=None)
...
cat_g = {}
for c in ['verbose_social','multi_intent_linked','general_conversational','code_technical']:
...
# H10: Total (input+output) token saving > 0
if len(tot_sav)>0:
    t10,p10 = ttest_1samp(tot_sav,0,alternative='greater')
    d10 = np.mean(tot_sav)/np.std(tot_sav,ddof=1) if np.std(tot_sav,ddof=1)>0 else 0
    ci10= stats.t.interval(0.95,df=len(tot_sav)-1,
                            loc=np.mean(tot_sav),scale=stats.sem(tot_sav))
    results['H10'] = dict(stat=t10,p=p10,d=d10,ci=ci10,n=len(tot_sav),
        name="Total (input+output) token saving > 0",
        dim="ECONOMY",method="One-sample t-test vs 0 (one-tailed)",
        sig=p10<ALPHA)

# H11: System prompt cache saving > 80%
cache_pct = (1 - CACHE_TOKS/SYS_TOKENS)*100 if SYS_TOKENS>0 else 0
results['H11'] = dict(stat=cache_pct,p=None,d=None,ci=None,
    n=len(dist_rows),
    name=f"Cache saves >{80}% of system prompt cost ({cache_pct:.0f}% observed)",
    dim="ECONOMY",method="Descriptive (cache_read_tokens vs system_tokens)",
    sig=(cache_pct>80))
...
for hid, r in results.items():
    ...
    status = ("PROVEN ✓" if r['sig'] is True else
              "NOT SIG" if r['sig'] is False else
              "PENDING")
...
if judge_clean:
    ...
    for ri3,(pid,jv) in enumerate(sorted(judge_clean.items()),5):
        ...
        vc(ws3,ri3,7,jv.get('reasoning','')[:120],alt,center=False,wrap=True)
...
# ── Final proof summary ───────────────────────────────────────────────────────
proven    = sum(1 for r in results.values() if r['sig'] is True)
not_sig   = sum(1 for r in results.values() if r['sig'] is False)
...
for dim in DIM_ORDER:
    ...
    dim_proven  = sum(1 for v in dim_results.values() if v['sig'] is True)
